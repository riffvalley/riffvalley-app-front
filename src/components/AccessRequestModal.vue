<template>
  <Teleport to="body">
    ...
    >
      <div
        class="w-[93%] sm:w-full sm:max-w-sm rounded-3xl bg-white shadow-2xl overflow-hidden relative"
      >
        <!-- Botón cerrar -->
        <button
          type="button"
          ...
          class="absolute top-3 right-3 z-10
            ...
            hover:bg-rv-pink hover:text-white
  ...
  <p class="mt-4 text-sm text-white/85 leading-relaxed text-left">
    ...
    Te contactaremos por correo para más instrucciones.
        ...
        <form @submit.prevent="submitRequest" class="p-5 space-y-4">
          <div>
            <label class="block text-sm font-semibold text-rv-navy mb-1">
              Email
            </label>
            <input
              v-model="email"
          ...
          <p v-if="success" class="text-green-600 text-sm text-center font-medium">
