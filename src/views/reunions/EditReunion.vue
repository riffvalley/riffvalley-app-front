<template>
  <div class="container mx-auto px-4 py-6 md:py-8">
    ...
    <div class="flex justify-between items-start gap-3 mb-4">
      <div class="min-w-0">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white truncate">{{ reunion.title }}</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">{{ formatDate(reunion.date) }}</p>
      </div>
      <button @click="toggleEditReunionForm"
        class="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 flex items-center shrink-0 transition-colors"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    return {
      ...
      updatePointReunion, cancelEdit, deletePoint, togglePointDone,
    };
  },
});
</script>
