          ...
          <article v-for="item in getItems(col.id)" :key="item.id" :draggable="canManage" class="group relative rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-white/10 dark:bg-rv-darkCard" :class="canManage ? 'cursor-move' : 'cursor-default'" @dragstart="onDragStart(item)">
            ...
            <div class="mt-3 flex items-center justify-between border-t border-gray-100 pt-2 dark:border-white/10">
              ...
              <div class="flex gap-2">
                ...
                <button v-if="!item.content && canManage" class="grid h-8 w-8 place-items-center rounded-lg bg-cyan-50 text-cyan-600 hover:bg-cyan-100 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-cyan-900/20 dark:text-cyan-400 dark:hover:bg-cyan-900/40" :disabled="!item.user" :title="item.user ? 'Añadir al calendario' : 'Asigna un usuario para poder añadir al calendario'" @click="handleCreateContent(item)"><i class="fa-solid fa-calendar-plus text-xs"></i></button>
              </div>
            </div>
          </article>
        </div>
...
async function handleCreateContent(item: Spotify) {
  ...
  try {
    ...
    SwalService.error(errorMessage(createError, 'No se pudo añadir al calendario'));
  }
}
