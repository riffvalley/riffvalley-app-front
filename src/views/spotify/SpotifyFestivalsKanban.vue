<template>
    <div class="p-4 md:p-6 min-h-[calc(100vh-64px)] flex flex-col dark:bg-rv-darkBg">
        <div class="flex items-center justify-between mb-6 flex-wrap gap-4">
            <h1 class="text-2xl md:text-3xl font-semibold dark:text-white"><i class="fa-solid fa-music mr-2"></i>Festivales</h1>
        ...
        <!-- Loading / Error -->
        <div v-if="loading" class="text-center py-10 dark:text-gray-400">Cargando festivales...</div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4"
            ...
            <div class="bg-white dark:bg-rv-darkCard rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
                <h3 class="text-lg font-bold dark:text-white">{{ isEditing ? 'Editar Festival' : 'Nuevo Festival' }}</h3>
...
const form = reactive({ name: '', link: '', updateDate: '' });
...
async function reload() {
    ...
    error.value = null;
    try {
        if (users.value.length === 0) users.value = await getUsersRv();
...
async function onDrop(targetState: ColumnId) {
    ...
    if (targetState === 'published') { draggedItem.value = null; return; }
    ...
    try {
        ...
        SwalService.error(e?.response?.data?.message || 'No se pudo actualizar el estado');
    } finally {
...
async function startEditingUser(itemId: string) {
    ...
    if (Array.isArray(selects) && selects.length > 0) selects[0]?.focus();
    else if (userSelectRef.value) (userSelectRef.value as HTMLSelectElement).focus();
}
...
async function save() {
    if (!form.name || !form.link) { SwalService.error('Completa todos los campos'); return; }
    try {
